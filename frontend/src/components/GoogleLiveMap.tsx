...
type Props = {
  location?: Location;
  route?: LatLng[];
  // Persisted takeoff position from the backend. When provided (even as
  // null = 'not recorded yet'), it is the ONLY source for the home marker,
  // so it survives page refreshes. When undefined, the old behaviour
  // (first position seen) is used.
  home?: LatLng | null;
  fallback?: ReactNode;
  /** True when there is no active telemetry signal (simulator paused/disconnected). */
  offline?: boolean;
  /** Message shown over the map while offline. */
...
export function LiveGoogleMap({ location, route, home: persistedHome, fallback, offline, offlineMessage }: Props) {
  ...
  useEffect(() => {
    // No active signal: stop recording new track points and wipe
    // whatever was recorded before the signal dropped, so a reconnect
    // (or the offline overlay itself) never shows a previous flight's
    // path lingering on the map.
    if (isOffline) {
      setTrack([]);
      setHome(null);
      return;
  ...
  const droneIcon = useMemo(() => L.divIcon({
    ...
    // Enlarged from 26x26 for visibility on the dashboard map.
    iconSize: [38, 38],
    iconAnchor: [19, 19],
  }), [current.heading]);
  ...
  const shownHome: LatLng | null = persistedHome !== undefined ? persistedHome : home;
  ...
  return (
    <div className="live-google-map idhtm-openstreet-map" aria-label="OpenStreetMap with live drone location">
      ...
      >
        ...
        {/* Only pan/track/mark the aircraft while there is an actual
            signal. While offline we freeze on whatever view was last
            shown instead of panning or redrawing stale data. */}
        {!isOffline && <RecenterMap location={current} />}
        {!isOffline && track.length > 1 && (
          <Polyline
            positions={track}
        ...
        {!isOffline && shownHome && (
          <Marker position={shownHome} icon={homeIcon}>
            <Tooltip direction="top" offset={[0, -30]} opacity={1}>
              Home / takeoff<br />
              {shownHome[0].toFixed(6)}°, {shownHome[1].toFixed(6)}°
            </Tooltip>
          </Marker>
        )}
        {!isOffline && <Marker position={position} icon={droneIcon} />}
      </MapContainer>

      {isOffline ? (
        <div className="live-map-offline-overlay" role="status">
          <div className="live-map-offline-badge">
            <WifiOff size={18} />
            <div>
              <strong>{offlineMessage || 'Simulator offline'}</strong>
              <span>No active telemetry signal — showing default map</span>
            </div>
          </div>
        </div>
      ) : (
      ...
      <div className="map-attribution">&copy; OpenStreetMap contributors {isOffline ? '' : '· live position'}</div>
    </div>
  );
}