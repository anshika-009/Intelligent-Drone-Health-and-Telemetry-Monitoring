...
if (!CLERK_PUBLISHABLE_KEY) {
  ReactDOM.createRoot(document.getElementById('root')!).render(
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', background: '#1a1d21', color: '#d9dfe5', fontFamily: 'sans-serif', textAlign: 'center', padding: '20px' }}>
      <h2 style={{ color: '#ff5a5f', marginBottom: '10px' }}>Missing Environment Variables</h2>
      <p>The application could not start because <code>VITE_CLERK_PUBLISHABLE_KEY</code> is missing.</p>
      <p style={{ marginTop: '15px' }}>Please add it to your <code>.env</code> file in the <code>frontend/</code> folder.</p>
    </div>
  );
...
ReactDOM.createRoot(document.getElementById('root')!).render(<React.StrictMode><ClerkProvider publishableKey={CLERK_PUBLISHABLE_KEY} afterSignOutUrl="/" appearance={{ baseTheme: dark }}><BrowserRouter><AppProvider><App/></AppProvider></BrowserRouter></ClerkProvider></React.StrictMode>);