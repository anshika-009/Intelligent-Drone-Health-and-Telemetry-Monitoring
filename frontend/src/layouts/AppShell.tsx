...
export function AppShell() {
  ...
  const { telemetry, scenario, simulatorActive, user, signOut } = useApp();
  ...
  return <div className="app-shell"><aside className={`sidebar ${open ? 'open' : ''}`}><div className="sidebar-head"><Brand/><button className="icon-button mobile-only" onClick={() => setOpen(false)} aria-label="Close navigation"><X size={17}/></button></div><div className="sidebar-drone"><div className="drone-avatar"><CircleGauge size={18}/></div><div><span>SELECTED AIRCRAFT</span><strong>DRONE-01 <ChevronDown size={14}/></strong></div></div><nav className="side-nav">{groups.map(group => <div className="nav-group" key={group.label}><span className="nav-group-label">{group.label}</span>{group.items.map(({ label, to, Icon }) => <NavLink key={label} to={to} onClick={() => setOpen(false)} className={({ isActive }) => `side-link ${isActive ? 'active' : ''}`}><Icon size={16}/><span>{label}</span>{label === 'Alerts' && <b>{telemetry.health_score < 80 ? '3' : '1'}</b>}</NavLink>)}</div>)}</nav><div className="sidebar-foot"><div className="connection-mini"><span className="live-dot"/><div><strong>{simulatorActive ? 'Simulator active' : 'Telemetry paused'}</strong><small>{scenario.replace('_', ' ')} / 1 Hz</small></div></div><button className="side-link logout" onClick={() => { signOut(); navigate('/'); }}><LogOut size={16}/>Sign out</button></div></aside><div className="app-main"><header className="topbar"><button className="icon-button mobile-only" onClick={() => setOpen(true)} aria-label="Open navigation"><Menu size={18}/></button><div className="breadcrumb"><span>Operations</span><strong> / {title}</strong></div><div className="top-actions"><div className="top-status"><StatusPill label={simulatorActive ? 'Simulator active' : 'Offline'} tone={simulatorActive ? 'green' : 'gray'}/><span className="telemetry-pulse"><span className="live-dot"/> Telemetry 1 Hz</span></div><button className="icon-button has-badge" onClick={() => navigate('/app/alerts')} aria-label="Notifications"><Bell size={18}/><b>{telemetry.health_score < 80 ? 3 : 1}</b></button><div className="user-menu"><span className="user-avatar">{(user?.name || 'FO').slice(0, 2).toUpperCase()}</span><span className="user-name">{user?.name || 'Flight operator'}</span></div></div></header><main className="app-content"><Outlet/></main></div></div>;
}
