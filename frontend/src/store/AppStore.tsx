...
const LOCATION_WS_URL =
  import.meta.env.VITE_LOCATION_WS_URL ||
  (import.meta.env.VITE_WS_URL || "ws://localhost:8000/ws/telemetry").replace(
    "/ws/telemetry",
    "/ws/drone-location",
  );
type Store = {
...
//   //     explanation:
//   //       "Remaining reserve is approaching the configured mission threshold.",
//   //     metric: `${telemetry.battery_percentage.toFixed(0)}%`,
//   //     recommendation:
//   //       "Plan a return and recharge the battery before the next flight.",
//   //     acknowledged: false,
//   //   });
  if (telemetry.signal_strength < 45)
    next.push({
      ...
      explanation: "Link reliability is decreasing progressively during this session.",
      metric: `${telemetry.signal_strength.toFixed(0)}%`,
      recommendation: "Prepare Return-To-Home.",
      acknowledged: false,
    });
  if (telemetry.vibration > 0.48)
    next.push({
      id: "LIVE-MOTOR",
      timestamp: "just now",
      severity: "CRITICAL",
      ...
      explanation: "Motor vibration has crossed the bearing inspection threshold.",
      metric: `${telemetry.vibration.toFixed(2)} g`,
      recommendation: "Land safely and inspect motor bearing before another extended flight.",
      acknowledged: false,
    });
  if (!telemetry.gps_fix)
    next.push({
      ...
      explanation: "The aircraft is not receiving a valid position solution.",
      metric: "0 satellites",
      recommendation: "Hold position if safe and initiate Return-To-Home.",
      acknowledged: false,
    });
  return next;
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [user, setUser] = useState<{ name: string; email: string } | null>(
    () => {
      const raw = localStorage.getItem("idhtm-user");
  ...
  useEffect(() => {
    ...
    try {
      ...
      socket.onmessage = (event) => {
        ...
        if (payload.alerts)
          setAlerts((previous) => [
            ...payload.alerts!.map((incoming) => {
              const existing = previous.find((p) => p.id === incoming.id);
              return existing
                ? { ...incoming, acknowledged: existing.acknowledged }
                : incoming;
            }),
            ...previous.filter((a) => a.id.startsWith("LIVE-")),
          ]);
      };
      socket.onerror = () => {
        // startFallback();
  ...
  useEffect(() => {
    ...
    setAlerts((previous) => {
      ...
      if (hasBackendAlerts)
        return previous.filter((a) => !a.id.startsWith("LIVE-"));
  ...
  const value = useMemo(
    () => ({
      ...
      user,
      signIn: (email: string, name = "Flight Operator") => {
        const next = { email, name };
        localStorage.setItem("idhtm-user", JSON.stringify(next));
        setUser(next);
      },
      signOut: () => {
        localStorage.removeItem("idhtm-user");
        setUser(null);
      },
    }),
    [telemetry, scenario, simulatorActive, alerts, user],
  );
